...
## v3.1 — Clean archive module

- Rebuilt with the NFT Terminal's exact shell, frame, header, mascot, colors, spacing, and LIVE panel.
- Removed every NFT Collection-specific section and widget.
- Added only archive boot output, commands, prompt, and archive footer.
- Uses one module title: `NFT Eligibility Archive`.
- Uses the prompt `hoodrat@eligibility:~$`.


## v4.0.1 — Correct archive application

- Uses the actual uploaded Eligibility Archive application.
- Preserves `archiveOutput`, `archiveForm`, `archiveInput`, and `bootArchive()`.
- Preserves archive commands: `help`, `about`, `nft`, and `clear`.
- Uses the finalized Landing Page header, LIVE panel, frame, and footer.
- No NFT Collection Terminal data panels are included.


## Market data runtime fix

The Eligibility Archive now uses the Landing Page's complete market backend:

- same DexScreener pair selection;
- same holder-count lookup;
- same market cache object;
- same `/api/price` response;
- same Price, Market Cap, Holders, and 24h Volume formatting.

Archive layout, commands, and eligibility data were not changed.
